#  A Bidirectional A* Algorithm for Solving Maze Problems
...
class A_Star:
    ...
    def __init__(self, root, s_x, s_y, e_x, e_y, w=40, h=20):
    ...
    def new(self):
        ...
        node = self.canvas.create_rectangle(580 - self.__r,
                                            20 - self.__r, 580 + self.__r, 20 + self.__r,
                                            fill="#7CCD7C",
                                            outline="#ffffff",
                                            tags="node",
                                            )
        self.canvas.create_text(630, 20,
                                text=u'Searched',
                                fill='black'
                                )
    ...
    def find_path(self):
        ...
        q = Node_Elem(None, self.e_x, self.e_y, 0.0)
        while True:
            # print('p {}'.format(self.open))
            # print('q {}'.format(self.open_q))
            # 扩展节点
            self.extend_round(p, self.open, self.close)
            self.extend_round(q, self.open_q, self.close_q)
            # for i in self.open:
            #     print('px {}, py {}'.format(i.x, i.y))
            # for j in self.open_q:
            #     print('qx {}, qy {}'.format(j.x, j.y))
            # 如果open表为空，则不存在路径，返回
            if not self.open or not self.open_q:
                return
            # 取F值最小的节点
            idx, p, cost = self.get_best(q)
            print(p.x, p.y)
            idx_q, q, cost_q = self.get_best_q(p)
            print(q.x, q.y)
            # 到达终点，生成路径，返回
            if self.is_target(p, q):
                self.make_path(p, self.path)
                self.make_path(q, self.path_q)
                print('cost', cost + cost_q - 1)
                return
            # 把此节点加入close表，并从open表里删除
            self.close.append(p)
            self.close_q.append(q)
            del self.open[idx]
            del self.open_q[idx_q]

    # 生成路径
    def make_path(self, p, path):
    ...
    def get_best_q(self, p):
        best = None
        bv = 10000000  # MAX值
        bi = -1
        cost = 0
        for idx, i in enumerate(self.open_q):
            value = self.get_dist_q(i, p)
            if value < bv:
                best = i
                bv = value
                bi = idx
    ...
    # 求距离
    def get_dist(self, i, q):
        # F = G + H
        # G 为当前路径长度(i.dist)，H为估计长度(该点到目标点的欧式距离)
        #
        return i.dist + math.sqrt((q.x - i.x) * (q.x - i.x) + (q.y - i.y) * (q.y - i.y))
        # return i.dist + math.sqrt((self.e_x - i.x) * (self.e_x - i.x) + (self.e_y - i.y) * (self.e_y - i.y))
        ##

    def get_dist_q(self, i, p):
        # F = G + H
        # G 为当前路径长度(i.dist)，H为估计长度(该点到目标点的欧式距离)
        #
        return i.dist + math.sqrt((p.x - i.x) * (p.x - i.x) + (p.y - i.y) * (p.y - i.y))
        # return i.dist + math.sqrt((self.s_x - i.x) * (self.s_x - i.x) + (self.s_y - i.y) * (self.s_y - i.y))

    # 扩展节点
    def extend_round(self, p, open_list, close_list):
        # 八个方向移动
        xs = (-1, 0, 1, -1, 1, -1, 0, 1)
        ys = (-1, -1, -1, 0, 0, 1, 1, 1)
        # 上下左右四个方向移动
        ...
        for x, y in zip(xs, ys):
            ...
            # 新节点在关闭列表，则忽略
            if self.node_in_close(node, close_list):
                continue
            i = self.node_in_open(node, open_list)
            # 新节点在open表
            if i != -1:
                # 当前路径距离更短
                if open_list[i].dist > node.dist:
                    # 更新距离
                    open_list[i].parent = p
                    open_list[i].dist = node.dist
    ...
    # 检查节点是否在close表
    def node_in_close(self, node, close_list):
        for i in close_list:
    ...
    # 检查节点是否在open表，返回序号
    def node_in_open(self, node, open_list):
...
def find_path():
    ...
    print(u"路径长度:%d" % (len(path) + len(path_q) - 1))
